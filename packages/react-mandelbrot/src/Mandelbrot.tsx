...
const Mandelbrot = ({
    ...
    const click = useCallback(
        (canvasX: number, canvasY: number, zoom: number) => {
            ...
            const y1 = canvasY * lenX * zoom + position.y1 - lenY / 2;
    ...
    const mouseDownHandler = useCallback(
        (e: MouseEvent) => {
            ...
            if (e.button == 0)
                return click(canvasX, canvasY, ZOOM_MULTIPLIER.CLICK_ZOOM_IN);
            if (e.button == 2)
                return click(canvasX, canvasY, ZOOM_MULTIPLIER.CLICK_ZOOM_OUT);
        },
        [click],
    );
    useEffect(() => {
