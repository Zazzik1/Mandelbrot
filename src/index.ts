...
const iterationsDatalist = document.querySelector(
    '#iterations',
) as HTMLSelectElement;
const colorPaletteSelectElement = document.querySelector(
    '#color-palette',
) as HTMLSelectElement;
const colorConvergedInput = document.querySelector(
    '#color-converged',
) as HTMLInputElement;
...
function click(
    rX: number,
    rY: number,
    mult: ZOOM_MULTIPLIER = ZOOM_MULTIPLIER.CLICK_ZOOM_IN,
) {
...
function addListeners() {
    canvas.addEventListener('mousedown', (e) => {
        ...
        if (e.button == 2)
            return click(
                e.offsetX / canvas.width,
                e.offsetY / canvas.height,
                ZOOM_MULTIPLIER.CLICK_ZOOM_OUT,
            );
    });
    ...
    canvas.addEventListener('wheel', (e) => {
        ...
        if (e.deltaY < 0) {
            click(
                e.offsetX / canvas.width,
                e.offsetY / canvas.height,
                ZOOM_MULTIPLIER.SCROLL_ZOOM_IN,
            );
        } else
            click(
                e.offsetX / canvas.width,
                e.offsetY / canvas.height,
                ZOOM_MULTIPLIER.SCROLL_ZOOM_OUT,
            );
    });
    ...
    document
        .getElementById('zoom_plus')
        ?.addEventListener('click', (e) => click(0.5, 0.5));
    document
        .getElementById('zoom_minus')
        ?.addEventListener('click', (e) => click(0.5, 0.5, 0.5));
    document
    ...
    colorPaletteSelectElement.addEventListener('change', () => {
        mandelbrot.setColorPalette(
            RGB_PALETTES[colorPaletteSelectElement.value],
        );
        draw();
    });
    ...
    INPUTS.CSIZE.addEventListener('change', () => {
        ...
        input.set(
            INPUTS.LEN2,
            (input.get(INPUTS.LEN) / canvas.width) * canvas.height,
        );
        draw();
    });

    /**
     * reload the canvas based on URL params when the Back/Forward
     * browser navigation buttons are clicked
     */
